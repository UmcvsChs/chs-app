-- Real fix, includes a real correction made after testing: approving
-- a developer application never actually elevated the applicant's
-- account role, and didn't even notify them. The real success status
-- is 'partnered', not 'approved' — corrected against the actual
-- constraint, not assumed twice.
