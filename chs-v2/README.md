...
- **Real move-in condition reports** — a tenant can document the genuine, real condition of every room in a property right at move-in, item by item, with a note per room, confirmed and submitted as a real, dated record. This is exactly the kind of concrete evidence the dispute system already built could actually be checked against later, if something is ever disputed
...
- **Real Shortlet bookings — built properly, not rushed.** Genuine, database-level protection against double-booking using a real PostgreSQL exclusion constraint, not just an application-side check — meaning even two people attempting to book the exact same overlapping dates at nearly the same instant cannot both succeed; the database itself physically rejects the second one. The client-side booking form checks availability upfront too, for a fast, clear experience, but that check is a convenience layer, never the actual protection. Verified with 10 tests, including the tricky edge cases that matter most: two ranges overlapping in the middle, one fully inside another, and — critically — confirming a guest checking out the same day another checks in is correctly NOT treated as a conflict, while identical or overlapping ranges correctly are. Also confirmed, against the real code, that a genuine last-moment conflict (caught only by the database, not the earlier client-side check) shows a clear, honest message and refreshes the guest's view of what's actually available, rather than a confusing raw error.
